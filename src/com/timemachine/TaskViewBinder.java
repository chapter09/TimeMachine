...
class TaskViewBinder implements SimpleCursorAdapter.ViewBinder
{
    public boolean setViewValue(View view, Cursor cursor, int columnIndex)
    {
        ...
        if(viewId == R.id.checkBox)
        {
            ...
            if(cursor.getInt(cursor.getColumnIndexOrThrow(TaskProvider.STATE)) == 1)
            {
                // Check CheckBox
                cb.setChecked(true);
                // Add a Striketrough
            ...
            cb.setText(cursor.getString(cursor.getColumnIndexOrThrow(TaskProvider.TODO)));

            return true;
        }
        else if(viewId == R.id.colorBar)
        {
            int color = cursor.getInt(cursor.getColumnIndexOrThrow(TaskProvider.COLOR));
